...
class UNet(nn.Module):
    def __init__(self, cfg):
        ...
        ch, out_ch, ch_mult = (
            self.cfg["model_channel"],
            cfg["model"]["output_dim"],
            tuple(self.cfg["channel_mult"]),
        )
        num_res_blocks = self.cfg["num_blocks"]
        attn_resolutions = []
...
class Guide_UNet(nn.Module):
    def __init__(self, cfg):
        ...
        self.config = cfg["model"]["unet"]
        self.ch = self.config["model_channel"] * self.config["channel_mult_emb"]
        self.guidance_scale = self.config["guidance_scale"]
        self.unet = UNet(cfg=cfg)
        # feature embedding
        self.input_dim = cfg["model"]["input_dim"]
        self.home_proj1 = nn.Sequential(
            nn.Linear(self.input_dim, self.ch),
            nn.ReLU(),
    ...
    def forward(self, batch, x, t):
        # feature embedding
        home1, work, home2 = (
            batch[:, 0, : self.input_dim],
            batch[:, 24, : self.input_dim],
            batch[:, -1, : self.input_dim],
        )
        home1 = self.home_proj1(home1)
        work = self.work_proj(work)
        home2 = self.home_proj2(home2)
        emb = home1 + work + home2
        cond_noise = self.unet(x, t, emb)
        pred_noise = cond_noise
        return pred_noise
